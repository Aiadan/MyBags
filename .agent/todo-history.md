...
This file preserves the previous long-form `TODOs.md` content after `TODOs.md` was reduced to a concise live backlog on 2026-03-07.

Treat this file as historical reference only. Agents should not update it during normal task completion.
...
* ✅ extended query docs/help with `expansionID` attribute coverage, including `expansionID` value labels (`0..12`).
...
* draggin item from bags to bank to a different category firstly causes the item to change category in the bag, and only next drag causes the item to be moved to a bag. It should do both at the same time.

...
5) Migration/seeding: load from `db.categorizers` when present; for legacy globals, seed into the new per-categorizer buckets (e.g., custom ⇒ `custom`, equipment sets ⇒ `eq`, new ⇒ `new`), without needing to preserve old version/sequences. No migration from version 2 is required since that schema was never released; if future migration is needed, use a new SavedVariables name.

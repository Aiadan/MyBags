...
* ✅ bank layout now mirrors bag UX: removed bank scrolling viewport, bank frame auto-resizes to rendered content with screen-fit scaling, added bottom-right drag resize with the same live preview/threshold behavior as bags, resize updates both `bank-character` and `bank-account` column counts together, bank column defaults are now first-load only (persisted user values are no longer forced back to 4 on init), bank frame size is now locked while bank search filtering is active to prevent filter-triggered resize jitter, the bank top-right edit cog now uses the same template/placement styling as the bag cog (with a slight upward anchor adjustment for alignment), and bank item-button hover now maps to category drop-hint frames so drag highlight feedback remains responsive across items (not only header/title regions).

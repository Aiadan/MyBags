...
* [!] add boxes with explanations over categories when draggin an item. What I mean by that is that at some point in time for debuging purposes when I was dragging categories or items, I had a functionality added, something like "on drag over" for categories areas which was changing their colour so I would clearly see where an item would be dropped. But I did think at that time that this functionality could be used for UX purposes. So that depending on category and what is being dragged, both the category could indicate some information by changing colour, and maybe in some cases "text appearing with info what will happen when I drop an item there". However in some cases it would be good to even colour / add text for a given category even before the item is dragged over a category - an example of that would be Unassigned category which should show text explaining that dropping at item on that category will reset any manual association with another category (this functionality, what dragging item into unassigned does, should also be documented in README.md). The challange is that it should not hinder the ability to place item after/before another item (item button). With such functionality I would like to have:
  * unassgined handled as described above
  * when draggin over a category that is... dynamic? I am not sure if we still have the concept of "protected" (or maybe now it is a lack of function to handle reassignment - need to check before implementing) -> but then it should show red background with info . "You cannot assign an item to this category as it is dynamic". In other words add the effect when dragging to indicate that a given cateogry is protected so you cannot assign to it - ie red background, shield pickture and some small text? And when howevering over a category to which you can assign indicate with text that it will be assigned to this one (maybe blue background)

...
* ✅ bank layout now mirrors bag UX: removed bank scrolling viewport, bank frame auto-resizes to rendered content with screen-fit scaling, added bottom-right drag resize with the same live preview/threshold behavior as bags, resize updates both `bank-character` and `bank-account` column counts together, and bank column defaults are now first-load only (persisted user values are no longer forced back to 4 on init).

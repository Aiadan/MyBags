...
* ✅ simplified `FrameParameters.lua` scaling model to shared width fit + per-frame height fit: width scale is computed from combined visible bag+bank widths, height scales are computed separately for bags and bank, and final per-frame scales use `min(widthScale, frameHeightScale)` clamped to `[0.05, 1]`; bag-refresh paths in `main.lua` now reapply both container and bank scales (when bank is shown) so bank scale tracks bag-size changes under the shared-width model; dedicated pure-math coverage lives in `tests/frame_parameters_scale_test.lua`.
